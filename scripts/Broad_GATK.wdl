...
#  
import 'https://api.firecloud.org/ga4gh/v1/tools/broad-malaria-firecloud-methods:haplotypecaller_gatk3-scatter_intervals/versions/6/plain-WDL/descriptor' as HaplotypeCaller
#import '/Users/tfarrell/Work/broad_malaria_firecloud/gatk3_germline_snps_indels/haplotypecaller_gatk3-scatter_intervals.6.wdl' as HaplotypeCaller
...
workflow GATK3_Germline_Variants {
    ...
    # preemptible 
    Int small_preemptible
    Int med_preemptible
    Int large_preemptible 
    ...
    # haplotypecaller
    Boolean genotype_all_sites
    String haplotypecaller_erc_flag = if genotype_all_sites then "BP_RESOLUTION" else "GVCF"

    # variant quality control param
    # either "vqsr" or "hard_filtering"
    String variant_qc     
    Boolean do_indel_vqsr
    ...
    scatter(sample in read_tsv(sample_paths_file)) {
        ...
        call HaplotypeCaller.HaplotypeCallerGvcf_GATK3 {
            ...
            haplotypecaller_erc_flag = haplotypecaller_erc_flag, 
            ...
            merge_gvcfs_preemptible = small_preemptible,
            haplotypecaller_preemptible = med_preemptible,
    ...
    scatter (interval in read_lines(interval_list)) {
        ...
        if (variant_qc == "vqsr") { 
            ...
            call VQSR as SnpVQSR {
                ...
                gvcf_index = GenotypeGVCFs.out_index,
        ...
        if (variant_qc == "hard_filtering") { 
            call HardFiltration { 
                ...
                vcf_index = GenotypeGVCFs.out_index,
    ...
    call GatherVCFs {
        input: 
        vcf_files = select_all(flatten([IndelVQSR.out, SnpVQSR.out, HardFiltration.out])),
        gitc_docker = gitc_docker,
        picard_path_gitc = gitc_path_to_picard,
    ...
    if (do_snpeff) {
        call SnpEff {
            ...
            vcf = GatherVCFs.out,
            cmdline_options = snpeff_options, 
            mem_size_gb = extra_large_mem_size_gb, 
    ...
    output { 
        File gvcf = select_first([SnpEff.out, GatherVCFs.out])
    }
}
...
task GenotypeGVCFs {
    ...
    String gcvf_out_index = "genotyped_gvcfs.vcf.gz.tbi"
    ...
    command {
        java -Xmx${cmd_mem_size_gb}G -jar ${gatk_path} \
            ...
            -o ${gcvf_out} \
            --variant ${vcf_file} \
            ${true="--includeNonVariantSites" false="" all_sites}
    }
    output {
       File out = gcvf_out
       File out_index = gcvf_out_index
    } 
...
task SnpEff {
    ...
    String? cmdline_options = "-no-downstream -no-intergenic -no-upstream -no-utr -noStats -treatAllAsProteinCoding false"
